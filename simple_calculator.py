...
def submit_number(number):
    ...
    #If demimal was pressed, disable the demimal button so it cannot be pressed
    if "." in display.get():
...
def operate(operator):
    ...
    #Disable all operator buttons until equal or clear are pressed
...
def equal():
    ...
    #Perform the desired mathmatics
    if operation == 'add':
    ...
    #Remove the current value of the display and update with the answer
    display.delete(0, END)
...
def square():
    """Calulate the square of a given number."""
